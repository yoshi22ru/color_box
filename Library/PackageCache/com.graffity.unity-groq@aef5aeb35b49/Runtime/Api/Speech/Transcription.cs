...
{
    /// <summary>
    /// Speech(AudioFile) to Text
    /// </summary>
    public class Transcription : IAsyncSpeechRequest<(long statusCode, SpeechResponse response)>
    {
        private readonly SpeechResponse CanceledResponse = new SpeechResponse(){Text= "Canceled"};
        ...
        public async UniTask<(long statusCode, SpeechResponse response)> SendAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                using(UnityWebRequest request = UnityWebRequest.Post(api.Endpoint, form))
                {
                    ...
                    Debug.Log($"Competed api call: {request.result}");
        ...
        /// </summary>
        /// <remarks>Required</remarks>
        /// <remarks>Only whisper-large-v3 is currently available.</remarks>
        /// <see cref="https://console.groq.com/docs/api-reference#audio-transcription"/>
        /// <param name="modelType"></param>
        /// <returns>this</returns>
        public IAsyncSpeechRequest<(long statusCode, SpeechResponse response)> SetModel(SpeechAiModelType modelType)
        {
            if (modelType != SpeechAiModelType.Whisper_large_v3) throw new NotSupportedException("Only whisper-large-v3 is currently available.");
